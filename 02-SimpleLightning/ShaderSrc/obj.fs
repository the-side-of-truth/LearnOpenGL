...
void main()
{
    ...
    vec3 emitColor = emit * vec3(texture(m.emitTex, TexPos + vec2(emitOffset, 0.0f))) * vec3(texture(m.specularTex, TexPos));

    // 累加结果
    vec3 result = emitColor + spotColor + dirColor + pointColor; 
...
vec3 spotLihgtRender(SpotLight _light, Material _m, vec3 _WPos, vec3 _WNormal, vec3 _CamCenter){
    ...
    // 读取材质纹理
    vec3 diffuseTexColor = vec3(texture(_m.diffuseTex, TexPos));
    vec3 specularTexColor = vec3(texture(_m.specularTex, TexPos));
